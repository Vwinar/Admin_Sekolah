    ...
    if ($full_name === '' || $username === '' || $role === '' || ($id === '' && $password === '') || $nip === '') {
        ...
        if ($existing_user && $existing_user['id'] != $id) {
            ...
            if ($id === '') {
                ...
                $stmt_insert = $db->prepare('INSERT INTO users (full_name, username, nip, password, role) VALUES (:full_name, :username, :nip, :password_hash, :role)');
                $stmt_insert->bindValue(':full_name', $full_name, PDO::PARAM_STR);
                $stmt_insert->bindValue(':username', $username, PDO::PARAM_STR);
                $stmt_insert->bindValue(':nip', $nip, PDO::PARAM_STR);
                $stmt_insert->bindValue(':password_hash', $password_hash, PDO::PARAM_STR);
                $stmt_insert->bindValue(':role', $role, PDO::PARAM_STR);
                ...
                if ($password !== '') {
                    $password_hash = password_hash($password, PASSWORD_DEFAULT);
                    $stmt_update = $db->prepare('UPDATE users SET full_name = :full_name, username = :username, nip = :nip, password = :password_hash, role = :role WHERE id = :id');
                    $stmt_update->bindValue(':password_hash', $password_hash, PDO::PARAM_STR);
                } else {
                    $stmt_update = $db->prepare('UPDATE users SET full_name = :full_name, username = :username, nip = :nip, role = :role WHERE id = :id');
                }
                $stmt_update->bindValue(':full_name', $full_name, PDO::PARAM_STR);
                $stmt_update->bindValue(':username', $username, PDO::PARAM_STR);
                $stmt_update->bindValue(':nip', $nip, PDO::PARAM_STR);
                $stmt_update->bindValue(':role', $role, PDO::PARAM_STR);
...
// Fetch users - EXCLUDE siswa role
$users = [];
$result = $db->query("SELECT id, full_name, username, nip, role FROM users WHERE role IN ('guru', 'admin') ORDER BY id ASC");
...
<body class="min-h-screen p-4 md:p-6">
    <div class="card w-full max-w-6xl mx-auto p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Manage Users,
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-8">
            ...
            <form method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="full_name" name="full_name"
                                value="<?= htmlspecialchars($edit_user['full_name'] ?? '') ?>" placeholder="Full name"
                                class="input-field w-full pl-10 pr-3 py-2 border border-gray-300" required />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="username" name="username"
                                value="<?= htmlspecialchars($edit_user['username'] ?? '') ?>" placeholder="Username"
                                class="input-field w-full pl-10 pr-3 py-2 border border-gray-300" required />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="nip" name="nip"
                                value="<?= htmlspecialchars($edit_user['nip'] ?? '') ?>" placeholder="NIP"
                                class="input-field w-full pl-10 pr-3 py-2 border border-gray-300" required />
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select id="role" name="role"
                                class="input-field w-full pl-10 pr-3 py-2 border border-gray-300 appearance-none"
                                required>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1" for="password">
                            Password
                            <?= $edit_user ? '<span class="text-gray-500 text-xs">(leave blank to keep current)</span>' : '' ?>
                        </label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <div class="flex items-center space-x-4 pt-2">
                    ...
                    <?php if ($edit_user): ?>
                        <a href="manage_guru_absensi.php" class="text-gray-600 hover:text-gray-800 hover:underline">
                            ...
                            <?php foreach ($users as $index => $user): ?>
                                <tr class="<?= $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' ?> hover:bg-gray-100">
                                    ...
                                    <td class="p-3 border-b border-gray-200 action-buttons">
                                        <a href="manage_guru_absensi.php?edit=<?= $user['id'] ?>"
                                            class="inline-flex items-center text-blue-600 hover:text-blue-800 mr-4">
                                            <i class="fas fa-edit mr-1"></i> Edit
                                        </a>
                                        <button class="inline-flex items-center btn-danger text-white py-1 px-3 rounded text-sm"
    ...
    <script>
        function showDeleteConfirmation(userId) {
            ...
            if (confirmation) {
                window.location.href = 'manage_guru_absensi.php?delete=' + userId;
            }
        }
    </script>
</body>
