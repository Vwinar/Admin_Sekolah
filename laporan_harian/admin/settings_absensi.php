...
// Ensure school_logo column exists
try {
    $db->query("SELECT school_logo FROM settings LIMIT 1");
} catch (PDOException $e) {
    if (strpos($e->getMessage(), 'no such column') !== false) {
        $db->exec("ALTER TABLE settings ADD COLUMN school_logo TEXT");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check existing
    $stmt = $db->query('SELECT school_logo FROM settings WHERE id = 1');
    $existing = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($existing) {
        $existing_logo_path = $existing['school_logo'] ?? '';
    }
    ...
    if ($exists) {
        // Update
        $sql = "UPDATE settings SET latitude = ?, longitude = ?, radius = ?, waktu_masuk = ?, waktu_pulang = ?, waktu_pulang_otomatis = ?, school_name = ?, school_logo = ? WHERE id = 1";
        $stmt = $db->prepare($sql);
        $stmt->execute([$latitude, $longitude, $radius, $waktu_masuk, $waktu_pulang, $waktu_pulang_otomatis, $school_name, $logo_path]);
    } else {
        // Insert
        // Assuming id is auto increment or we force it to 1? The original code forces id=1.
        $sql = "INSERT INTO settings (id, latitude, longitude, radius, waktu_masuk, waktu_pulang, waktu_pulang_otomatis, school_name, school_logo) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        $stmt->execute([$latitude, $longitude, $radius, $waktu_masuk, $waktu_pulang, $waktu_pulang_otomatis, $school_name, $logo_path]);
    }
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 rounded shadow">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <a href="admin_absensi.php"
                class="mt-4 md:mt-0 inline-flex items-center justify-center bg-gray-800 text-white py-2 px-4 rounded-lg hover:bg-gray-700 transition">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6" id="settings-form">
            <div>
                ...
                <div id="map"></div>
                <p class="text-sm text-gray-500 mb-4">Klik pada peta untuk menentukan lokasi atau masukkan koordinat
                    manual</p>
            </div>
            ...
            <div>
                ...
                <div class="flex space-x-2 items-center">
                    <input type="text" id="latitude" name="latitude" value="<?= htmlspecialchars($latitude) ?>"
                        class="flex-1 border border-gray-300 rounded p-2" required />
            ...
            <div>
                ...
                <input type="number" id="radius" name="radius" value="<?= htmlspecialchars($radius) ?>"
                    class="w-full border border-gray-300 rounded p-2" required min="0" />
            </div>

            <div>
                <label for="waktu_masuk" class="block font-medium mb-1">Waktu Masuk (Time In) - Format 24 Jam</label>
                <input type="time" id="waktu_masuk" name="waktu_masuk" value="<?= htmlspecialchars($waktu_masuk) ?>"
                    class="w-full border border-gray-300 rounded p-2" required step="1" />
            </div>

            <div>
                <label for="waktu_pulang" class="block font-medium mb-1">Waktu Pulang (Time Out) - Format 24 Jam</label>
                <input type="time" id="waktu_pulang" name="waktu_pulang" value="<?= htmlspecialchars($waktu_pulang) ?>"
                    class="w-full border border-gray-300 rounded p-2" required step="1" />
            </div>

            <div>
                <label for="waktu_pulang_otomatis" class="block font-medium mb-1">Waktu Pulang Otomatis (Automatic Time
                    Out) - Format 24 Jam</label>
                <input type="time" id="waktu_pulang_otomatis" name="waktu_pulang_otomatis"
                    value="<?= htmlspecialchars($waktu_pulang_otomatis) ?>"
                    class="w-full border border-gray-300 rounded p-2" step="1" />
            </div>

            <div>
                <label for="school_name" class="block font-medium mb-1">School Name</label>
                <input type="text" id="school_name" name="school_name" value="<?= htmlspecialchars($school_name) ?>"
                    class="w-full border border-gray-300 rounded p-2" required />
            </div>
                ...
                <?php if ($school_logo): ?>
                    <div class="relative inline-block mb-2">
                        ...
                        <button type="button" id="btn-remove-logo"
                            class="absolute top-0 right-10 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-700 transition"
                            title="Hapus Gambar">&times;</button>
                        <lab for="school_logo"
                            class="absolute top-0 right-0 bg-blue-600 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-blue-700 transition cursor-pointer"
                            title="Ganti Gambar">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M15.232 5.232l3.536 3.536M9 11l6 6m-6-6l-3 3m3-3l3-3m-3 3v6" />
                            </svg>
                            </label>
                    </div>
                <?php else: ?>
            ...
            <div class="flex space-x-4">
                <button type="submit"
                    class="bg-blue-700 text-white px-4 py-2 rounded hover:bg-red-800 transition">Simpan</button>
