...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $role = $_POST['role'] ?? '';
        $action_type = $_POST['action_type'] ?? 'absen';

        if ($username && $password && $role && $action_type) {
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="card w-full max-w-md p-8">
        ...
        <form method="POST" action="login.php" class="space-y-5" id="loginForm">
            ...
            <div class="flex w-full">
                <button type="submit" id="loginBtn" class="btn-primary flex-1 text-white py-2.5 rounded-l-lg font-medium text-sm" disabled>
                    Login
                </button>
                <button type="button" onclick="location.reload()" class="btn-primary flex-1 text-white py-2.5 rounded-r-lg font-medium text-sm">
    ...
    <script>
        ...
        function checkLoginEligibility() {
            ...
            if (role === 'admin' || actionType === 'izin') {
                loginBtn.disabled = false;
                errorMsg.classList.add('hidden');
                document.getElementById('distanceDisplay').textContent = (role === 'admin') ? 'Admin: Login tanpa batas jarak.' : 'Izin: Login tanpa batas jarak.';
            } else {
                if (!userLat || !userLng) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    // Check for potential GPS spoofing (skip for admin and izin)
                    const role = document.getElementById('role').value;
                    const actionType = document.getElementById('action_type').value;
                    if (role !== 'admin' && actionType !== 'izin' && accuracy > 200) { // If accuracy is poor (>100m), likely spoofed
                        document.getElementById('locationError').textContent = 'Lokasi tidak akurat. Kemungkinan pelangggaraan. Login tidak dapat dilakukan.';
            ...
            // Re-check when role or action_type changes
            roleSelect.addEventListener('change', checkLoginEligibility);
            const actionTypeSelect = document.getElementById('action_type');
            actionTypeSelect.addEventListener('change', checkLoginEligibility);
        });
    </script>
</body>
</html>